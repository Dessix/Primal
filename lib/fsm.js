...
(function (FSM) {
    ...
    })(FSM.TransitionKind || (FSM.TransitionKind = {}));
    var TransitionKind = FSM.TransitionKind;
    /**
    ...
    function evaluate(model, instance, message, autoInitialiseModel) {
        ...
        // log as required
        FSM.console.log(instance + " evaluate " + message);
    ...
    function evaluateState(state, instance, message) {
        ...
        if (result) {
            ...
            if (transitions.length === 1) {
                ...
                FSM.console.error(state + ": multiple outbound transitions evaluated true for message " + message);
            }
        }
        return result;
    }
    ...
    function selectTransition(pseudoState, instance, message) {
        ...
        if (pseudoState.kind === PseudoStateKind.Choice) {
            return results.length !== 0 ? results[FSM.random(results.length)] : findElse(pseudoState);
        }
        else {
            if (results.length > 1) {
                FSM.console.error("Multiple outbound transition guards returned true at " + pseudoState + " for " + message);
            }
            else {
    ...
    var InitialiseTransitions = (function (_super) {
        ...
        InitialiseTransitions.prototype.visitInternalTransition = function (transition, behavior) {
            ...
            if (FSM.internalTransitionsTriggerCompletion) {
    ...
    var InitialiseElements = (function (_super) {
        ...
        InitialiseElements.prototype.visitElement = function (element, deepHistoryAbove) {
            if (FSM.console !== defaultConsole) {
                this.behavior(element).leave.push(function (message, instance) { return FSM.console.log(instance + " enter " + element); });
                this.behavior(element).beginEnter.push(function (message, instance) { return FSM.console.log(instance + " enter " + element); });
            }
        };
    ...
    var Validator = (function (_super) {
        ...
        Validator.prototype.visitPseudoState = function (pseudoState) {
            ...
            if (pseudoState.kind === PseudoStateKind.Choice || pseudoState.kind === PseudoStateKind.Junction) {
                ...
                if (pseudoState.outgoing.length === 0) {
                    FSM.console.error(pseudoState + ": " + pseudoState.kind + " pseudo states must have at least one outgoing transition.");
                }
                ...
                if (pseudoState.outgoing.filter(function (transition) { return transition.guard === Transition.FalseGuard; }).length > 1) {
                    FSM.console.error(pseudoState + ": " + pseudoState.kind + " pseudo states cannot have more than one Else transitions.");
                }
            }
            else {
                ...
                if (pseudoState.outgoing.filter(function (transition) { return transition.guard === Transition.FalseGuard; }).length !== 0) {
                    FSM.console.error(pseudoState + ": " + pseudoState.kind + " pseudo states cannot have Else transitions.");
                }
                if (pseudoState.isInitial()) {
                    if (pseudoState.outgoing.length > 1) {
                        // [1] An initial vertex can have at most one outgoing transition.
                        // [2] History vertices can have at most one outgoing transition.
                        FSM.console.error(pseudoState + ": initial pseudo states must have no more than one outgoing transition.");
                    }
                        ...
                        if (pseudoState.outgoing[0].guard !== Transition.TrueGuard) {
                            FSM.console.error(pseudoState + ": initial pseudo states cannot have a guard condition.");
                        }
                    }
                }
            }
        ...
        Validator.prototype.visitRegion = function (region) {
            ...
            if (initial > 1) {
                FSM.console.error(region + ": regions may have at most one initial pseudo state.");
            }
            if (deepHistory > 1) {
                FSM.console.error(region + ": regions may have at most one deep history pseudo state.");
            }
            if (shallowHistory > 1) {
                FSM.console.error(region + ": regions may have at most one shallow history pseudo state.");
            }
        };
        Validator.prototype.visitState = function (state) {
            ...
            if (state.regions.filter(function (region) { return region.name === Region.defaultName; }).length > 1) {
                FSM.console.error(state + ": a state cannot have more than one region named " + Region.defaultName);
            }
        };
        Validator.prototype.visitFinalState = function (finalState) {
            ...
            if (finalState.outgoing.length !== 0) {
                FSM.console.error(finalState + ": final states must not have outgoing transitions.");
            }
            // [2] A final state cannot have regions.
            if (finalState.regions.length !== 0) {
                FSM.console.error(finalState + ": final states must not have child regions.");
            }
            // [4] A final state has no entry behavior.
            if (finalState.entryBehavior.length > 0) {
                FSM.console.warn(finalState + ": final states may not have entry behavior.");
            }
            // [5] A final state has no exit behavior.
            if (finalState.exitBehavior.length > 0) {
                FSM.console.warn(finalState + ": final states may not have exit behavior.");
            }
        };
        Validator.prototype.visitTransition = function (transition) {
            ...
            if (transition.kind === TransitionKind.Local) {
                if (transition.target.ancestry().indexOf(transition.source) === -1) {
                    FSM.console.error(transition + ": local transition target vertices must be a child of the source composite sate.");
                }
            }
        };
        return Validator;
